      ...
      <div class="flex h-12 font-medium rounded-full bg-cyberyellow">
        ...
        >
          <div
            ...
            class="text-black"
            @click="changePageOffers(index)"
          >
            {{ index }}
          </div>
          <div v-else @click="changePageOffers(index)">{{ index }}</div>
        </div>
        <div
          class="w-12 md:flex justify-center items-center cursor-pointer leading-5 transition duration-150 ease-in rounded-full"
        >
...
export default {
  ...
  methods: {
    ...mapActions("paginationBoard", ["changePageOffers"]),
  },
};
</script>
